...
pub extern "C" fn send_frame(buf: *const u8, len: usize) -> isize {
    // SAFETY: Caller guarantees `buf` is valid and readable for `len` bytes.
    // The pointer is passed as a plain integer to the kernel via ecall; the
    // kernel copies the data before returning, so no aliasing issues arise.
    syscall_fn(10, buf as usize, len, 0) as isize
}
...
pub extern "C" fn recv_frame(buf: *mut u8, len: usize) -> isize {
    // SAFETY: Caller guarantees `buf` is valid and writable for `len` bytes.
    // The pointer is passed as a plain integer to the kernel via ecall; the
    // kernel writes into it before returning.
    syscall_fn(11, buf as usize, len, 0) as isize
}

/// Safe Rust wrappers for higher-level use in no_std code.
pub fn send_frame_rs(packet: &[u8]) -> isize {
    syscall_fn(10, packet.as_ptr() as usize, packet.len(), 0) as isize
}

pub fn recv_frame_rs(buf: &mut [u8]) -> isize {
    syscall_fn(11, buf.as_mut_ptr() as usize, buf.len(), 0) as isize
}
